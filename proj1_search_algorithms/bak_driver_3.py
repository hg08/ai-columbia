...
def A_star_search(initial_state,goal_state):
    ...
    m = n*n
    g_score[initial_state] = 0  # A sequence
    h_score[initial_state] = distance(inital_state,goal_state)
    distance(inital_state,goal_state) = \
           sum([calculate_manhattan_dist(list(goal_state.config)[i],list(initial_state.config)[i],n) for i in range(m)])
    f_score(initial_state) = h_score(initial_state)
    ...
    while frontier._qsize():
        ...
        for neighbor in state.expand():
            ...
            if tentative_is_better == True:
                came_from(neighor) = state
                g_score(neighbor) = tentative_g_score
                h_score(neighbor) = distance(neighbor,goal)
